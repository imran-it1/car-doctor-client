		...
		signIn(email, password)
			.then(result => {
				...
				axios
					...
					.catch(error => {
						console.log(error);
						toast.error("Error from JWT", error);
					});
			})
			.catch(error => {
				console.log(error);
	...
	return (
		<div className="container px-6 m-auto py-10">
			<div className="grid grid-cols-4 gap-6 md:grid-cols-8 lg:grid-cols-12">
				<div className="col-span-4 lg:col-span-6  self-center mx-auto">
					<img src={img} alt="" />
				</div>
				<div className=" w-full col-span-4 lg:col-span-6 border rounded-lg p-16 items-center">
					<p className=" text-2xl font-bold flex justify-center items-center py-5">
						Log In
					</p>
					...
					<form onSubmit={handleLogIn} className=" w-full">
						...
						<button className="w-full">
							<input
								className="text-white bg-rose-500 hover:bg-rose-600 focus:ring-4 focus:outline-none focus:ring-rose-300 font-medium rounded-lg text-sm w-full  px-5 py-2.5 text-center dark:bg-rose-600 dark:hover-bg-rose-700 dark:focus:ring-rose-800 hover:cursor-pointer transition-all duration-300 ease-linear"
								type="submit"
								value="Log In"
							/>
						</button>

						<div className=" flex items-center flex-col pt-5 space-y-5">
							<p>Or Sign In with</p>
							{/* Social */}
